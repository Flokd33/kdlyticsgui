...
(defn close-trade-modal-sql []
  (let [exit-rationale (r/atom nil)
        last-leg-uuid ((keyword "uuid") (last (:trades @(rf/subscribe [:ta2022/trade-history]))))]
    (fn []
...
(defn morph-or-amend-trade-modal-sql [morph?]
  (let [last-trade (last (:trades @(rf/subscribe [:ta2022/trade-history])))
        last-leg-uuid ((keyword "uuid") last-trade)
        exit-rationale (r/atom nil)
        all-alerts (let [x (taalerts/trade->alerts-sql last-trade (:alerts @(rf/subscribe [:ta2022/trade-history])))] (zipmap (map (keyword "uuid") x) x))
        trade-entry
        (if morph?
          (if (and last-leg-uuid
                   (= (get-in all-alerts [((keyword "relval_alert_uuid") last-trade) (keyword  "alert_type")]) "spread")
                   (= (get-in all-alerts [((keyword "target_alert_uuid") last-trade) (keyword "alert_type")]) "single")
                   (= (get-in all-alerts [((keyword "review_alert_uuid") last-trade) (keyword "alert_type")]) "single"))
            (r/atom (merge
                      {:analyst         ((keyword  "analyst") last-trade)
                       :strategy        nil
                       :entry-rationale nil
                       :ISIN            @(rf/subscribe [:ta2022/trade-isin])
                       :relval-alert    (taalerts/alert-from-backend-sql (all-alerts ((keyword "relval_alert_uuid") last-trade)) ) ;issue here with relval
                       :target-alert    (taalerts/alert-from-backend-sql (all-alerts ((keyword "target_alert_uuid") last-trade)) )
                       :review-alert    (taalerts/alert-from-backend-sql (all-alerts ((keyword "review_alert_uuid") last-trade)) )}
                      {:other-alert-1    (taalerts/alert-from-backend-sql (all-alerts ((keyword "other_alert_uuid_1") last-trade)) )})
                    )
          ...
          (r/atom (merge {:analyst         ((keyword "analyst") last-trade)
                          :strategy        ((keyword "strategy") last-trade)
                          :entry-rationale ((keyword "entry_rationale") last-trade)
                          :ISIN            @(rf/subscribe [:ta2022/trade-isin])
                          :relval-alert    (taalerts/alert-from-backend-sql (all-alerts ((keyword "relval_alert_uuid") last-trade)))
                          :target-alert    (taalerts/alert-from-backend-sql (all-alerts ((keyword "target_alert_uuid") last-trade)))
                          :review-alert    (taalerts/alert-from-backend-sql (all-alerts ((keyword "review_alert_uuid") last-trade)))}
                         {:other-alert-1    (taalerts/alert-from-backend-sql (all-alerts ((keyword "other_alert_uuid_1") last-trade)) )})
                   ))
      ...
      [v-box :width "850px" :height "750px" :gap "10px" :padding "20px"
       :children [[h-box :align :center :children [(if morph? [title :label (if last-leg-uuid "Morph trade" "New trade") :level :level1] [title :label "Amend latest trade" :level :level1]) [gap :size "1"] [md-circle-icon-button :md-icon-name "zmdi-close" :on-click #(rf/dispatch [:ta2022/close-modal])]]]
                  ...
                  [h-box :align :center :children [[label :width "125px" :label "Current levels"]
                                                   [:> ReactTable {:data [(first (t/chainfilter {:ISIN @(rf/subscribe [:ta2022/trade-isin])} @(rf/subscribe [:quant-model/model-output])))] :columns (qstables/table-style->qs-table-col "TA2022morph" nil) :showPagination false :pageSize 1 :filterable false}]]]
...
(defn morph-trade-modal-sql [] (morph-or-amend-trade-modal-sql true))
(defn amend-latest-trade-modal-sql [] (morph-or-amend-trade-modal-sql false))
