...
(defn single-portfolio-risk-display []
  (let [                                                    ;portfolio-map @(rf/subscribe [:portfolio-dropdown-map])
        ...
        risk-choices-clean (if (= @(rf/subscribe [:single-portfolio-risk/portfolio]) "OGTRCRED") (if (= (some #{:sector} risk-choices) :sector) (vec (conj (remove #(= % :sector) risk-choices) :sector-baml)) risk-choices) risk-choices) ;TODO change for portfolio_strategy <> 'DM' =>  we want to change the key used for the sector classification when BAML sector needed
        grouping-columns (into [] (for [r (remove nil? (conj risk-choices-clean :name))] (tables/risk-table-columns r)))
