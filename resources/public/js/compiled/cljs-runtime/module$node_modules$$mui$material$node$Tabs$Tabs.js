...
_ownerWindow=_interopRequireDefault(require("module$node_modules$$mui$material$node$utils$ownerWindow")),_jsxRuntime=require("module$node_modules$react$jsx_runtime");const _excluded="aria-label aria-labelledby action centered children className component allowScrollButtonsMobile indicatorColor onChange orientation ScrollButtonComponent scrollButtons selectionFollowsFocus slots slotProps TabIndicatorProps TabScrollButtonProps textColor value variant visibleScrollbar".split(" "),nextItem=(list,item)=>
list===item?list.firstChild:item&&item.nextElementSibling?item.nextElementSibling:list.firstChild,previousItem=(list,item)=>list===item?list.lastChild:item&&item.previousElementSibling?item.previousElementSibling:list.lastChild,moveFocus=(list,currentFocus,traversalFunction)=>{let wrappedOnce=!1;for(currentFocus=traversalFunction(list,currentFocus);currentFocus;){if(currentFocus===list.firstChild){if(wrappedOnce)break;wrappedOnce=!0}const nextFocusDisabled=currentFocus.disabled||"true"===currentFocus.getAttribute("aria-disabled");
...
allowScrollButtonsMobile=!1,indicatorColor="primary",onChange,orientation="horizontal",ScrollButtonComponent=_TabScrollButton.default,scrollButtons="auto",selectionFollowsFocus,slots={},slotProps={},TabIndicatorProps={},TabScrollButtonProps={},textColor="primary",value,variant="standard",visibleScrollbar=!1}=props;inProps=(0,_objectWithoutPropertiesLoose2.default)(props,_excluded);const scrollable="scrollable"===variant,vertical="vertical"===orientation,scrollStart=vertical?"scrollTop":"scrollLeft",
start=vertical?"top":"left",end=vertical?"bottom":"right",clientSize=vertical?"clientHeight":"clientWidth",size=vertical?"height":"width";props=(0,_extends2.default)({},props,{component,allowScrollButtonsMobile,indicatorColor,orientation,vertical,scrollButtons,textColor,variant,visibleScrollbar,fixed:!scrollable,hideScrollbar:scrollable&&!visibleScrollbar,scrollableX:scrollable&&!vertical,scrollableY:scrollable&&vertical,centered:centered&&!scrollable,scrollButtonsHideMobile:!allowScrollButtonsMobile});
const classes=useUtilityClasses(props),startScrollButtonIconProps=(0,_base.useSlotProps)({elementType:slots.StartScrollButtonIcon,externalSlotProps:slotProps.startScrollButtonIcon,ownerState:props}),endScrollButtonIconProps=(0,_base.useSlotProps)({elementType:slots.EndScrollButtonIcon,externalSlotProps:slotProps.endScrollButtonIcon,ownerState:props});centered&&scrollable&&console.error('MUI: You can not use the `centered\x3d{true}` and `variant\x3d"scrollable"` properties at the same time on a `Tabs` component.');
const [mounted,setMounted]=React.useState(!1),[indicatorStyle,setIndicatorStyle]=React.useState(defaultIndicatorStyle),[displayStartScroll,setDisplayStartScroll]=React.useState(!1),[displayEndScroll,setDisplayEndScroll]=React.useState(!1),[updateScrollObserver,setUpdateScrollObserver]=React.useState(!1),[scrollerStyle,setScrollerStyle]=React.useState({overflow:"hidden",scrollbarWidth:0}),valueToIndex=new Map,tabsRef=React.useRef(null),tabListRef=React.useRef(null),getTabsMeta=()=>{var tabsNode=tabsRef.current;
if(tabsNode){var tabsMeta=tabsNode.getBoundingClientRect();tabsMeta={clientWidth:tabsNode.clientWidth,scrollLeft:tabsNode.scrollLeft,scrollTop:tabsNode.scrollTop,scrollLeftNormalized:(0,_scrollLeft.getNormalizedScrollLeft)(tabsNode,theme.direction),scrollWidth:tabsNode.scrollWidth,top:tabsMeta.top,bottom:tabsMeta.bottom,left:tabsMeta.left,right:tabsMeta.right}}var tabMeta;tabsNode&&!1!==value&&(tabsNode=tabListRef.current.children,0<tabsNode.length&&((tabMeta=tabsNode[valueToIndex.get(value)])||console.error(["MUI: The `value` provided to the Tabs component is invalid.",
`None of the Tabs' children match with "${value}".`,valueToIndex.keys?`You can provide one of the following values: ${Array.from(valueToIndex.keys()).join(", ")}.`:null].join("\n")),tabMeta=tabMeta?tabMeta.getBoundingClientRect():null,!warnedOnceTabPresent&&tabMeta&&0===tabMeta.width&&0===tabMeta.height&&0!==tabsMeta.clientWidth&&(tabsMeta=null,console.error(["MUI: The `value` provided to the Tabs component is invalid.",`The Tab with this \`value\` ("${value}") is not part of the document layout.`,
"Make sure the tab item is present in the document or that it's not `display: none`."].join("\n")),warnedOnceTabPresent=!0)));return{tabsMeta,tabMeta}},updateIndicatorState=(0,_useEventCallback.default)(()=>{const {tabsMeta,tabMeta}=getTabsMeta();var startValue=0;let startIndicator;vertical?(startIndicator="top",tabMeta&&tabsMeta&&(startValue=tabMeta.top-tabsMeta.top+tabsMeta.scrollTop)):(startIndicator=isRtl?"right":"left",tabMeta&&tabsMeta&&(startValue=(isRtl?-1:1)*(tabMeta[startIndicator]-tabsMeta[startIndicator]+
(isRtl?tabsMeta.scrollLeftNormalized+tabsMeta.clientWidth-tabsMeta.scrollWidth:tabsMeta.scrollLeft))));startValue={[startIndicator]:startValue,[size]:tabMeta?tabMeta[size]:0};if(isNaN(indicatorStyle[startIndicator])||isNaN(indicatorStyle[size]))setIndicatorStyle(startValue);else{const dSize=Math.abs(indicatorStyle[size]-startValue[size]);(1<=Math.abs(indicatorStyle[startIndicator]-startValue[startIndicator])||1<=dSize)&&setIndicatorStyle(startValue)}}),scroll=(scrollValue,{animation=!0}={})=>{animation?
(0,_animate.default)(scrollStart,tabsRef.current,scrollValue,{duration:theme.transitions.duration.standard}):tabsRef.current[scrollStart]=scrollValue},moveTabsScroll=delta=>{let scrollValue=tabsRef.current[scrollStart];vertical?scrollValue+=delta:(scrollValue+=delta*(isRtl?-1:1),scrollValue*=isRtl&&"reverse"===(0,_scrollLeft.detectScrollType)()?-1:1);scroll(scrollValue)},getScrollSize=()=>{const containerSize=tabsRef.current[clientSize];let totalSize=0;const children=Array.from(tabListRef.current.children);
for(let i=0;i<children.length;i+=1){const tab=children[i];if(totalSize+tab[clientSize]>containerSize){0===i&&(totalSize=containerSize);break}totalSize+=tab[clientSize]}return totalSize},handleStartScrollClick=()=>{moveTabsScroll(-1*getScrollSize())},handleEndScrollClick=()=>{moveTabsScroll(getScrollSize())},handleScrollbarSizeChange=React.useCallback(scrollbarWidth=>{setScrollerStyle({overflow:null,scrollbarWidth})},[]),scrollSelectedIntoView=(0,_useEventCallback.default)(animation=>{const {tabsMeta,
tabMeta}=getTabsMeta();tabMeta&&tabsMeta&&(tabMeta[start]<tabsMeta[start]?scroll(tabsMeta[scrollStart]+(tabMeta[start]-tabsMeta[start]),{animation}):tabMeta[end]>tabsMeta[end]&&scroll(tabsMeta[scrollStart]+(tabMeta[end]-tabsMeta[end]),{animation}))}),updateScrollButtonState=(0,_useEventCallback.default)(()=>{scrollable&&!1!==scrollButtons&&setUpdateScrollObserver(!updateScrollObserver)});React.useEffect(()=>{const handleResize=(0,_debounce.default)(()=>{tabsRef.current&&updateIndicatorState()}),win=
(0,_ownerWindow.default)(tabsRef.current);win.addEventListener("resize",handleResize);let resizeObserver;"undefined"!==typeof ResizeObserver&&(resizeObserver=new ResizeObserver(handleResize),Array.from(tabListRef.current.children).forEach(child=>{resizeObserver.observe(child)}));return()=>{handleResize.clear();win.removeEventListener("resize",handleResize);resizeObserver&&resizeObserver.disconnect()}},[updateIndicatorState]);React.useEffect(()=>{var tabListChildren=Array.from(tabListRef.current.children),
length=tabListChildren.length;if("undefined"!==typeof IntersectionObserver&&0<length&&scrollable&&!1!==scrollButtons){const firstTab=tabListChildren[0];tabListChildren=tabListChildren[length-1];length={root:tabsRef.current,threshold:.99};const firstObserver=new IntersectionObserver(entries=>{setDisplayStartScroll(!entries[0].isIntersecting)},length);firstObserver.observe(firstTab);const lastObserver=new IntersectionObserver(entries=>{setDisplayEndScroll(!entries[0].isIntersecting)},length);lastObserver.observe(tabListChildren);
return()=>{firstObserver.disconnect();lastObserver.disconnect()}}},[scrollable,scrollButtons,updateScrollObserver,null==childrenProp?void 0:childrenProp.length]);React.useEffect(()=>{setMounted(!0)},[]);React.useEffect(()=>{updateIndicatorState()});React.useEffect(()=>{scrollSelectedIntoView(defaultIndicatorStyle!==indicatorStyle)},[scrollSelectedIntoView,indicatorStyle]);React.useImperativeHandle(action,()=>({updateIndicator:updateIndicatorState,updateScrollButtons:updateScrollButtonState}),[updateIndicatorState,
updateScrollButtonState]);const indicator=(0,_jsxRuntime.jsx)(TabsIndicator,(0,_extends2.default)({},TabIndicatorProps,{className:(0,_clsx.default)(classes.indicator,TabIndicatorProps.className),ownerState:props,style:(0,_extends2.default)({},indicatorStyle,TabIndicatorProps.style)}));let childIndex=0;const children$jscomp$0=React.Children.map(childrenProp,child=>{if(!React.isValidElement(child))return null;(0,_reactIs.isFragment)(child)&&console.error("MUI: The Tabs component doesn't accept a Fragment as a child.\nConsider providing an array instead.");
const childValue=void 0===child.props.value?childIndex:child.props.value;valueToIndex.set(childValue,childIndex);const selected=childValue===value;childIndex+=1;return React.cloneElement(child,(0,_extends2.default)({fullWidth:"fullWidth"===variant,indicator:selected&&!mounted&&indicator,selected,selectionFollowsFocus,onChange,textColor,value:childValue},1!==childIndex||!1!==value||child.props.tabIndex?{}:{tabIndex:0}))}),conditionalElements=(()=>{const conditionalElements={};conditionalElements.scrollbarSizeListener=
scrollable?(0,_jsxRuntime.jsx)(TabsScrollbarSize,{onChange:handleScrollbarSizeChange,className:(0,_clsx.default)(classes.scrollableX,classes.hideScrollbar)}):null;var scrollButtonsActive=displayStartScroll||displayEndScroll;scrollButtonsActive=scrollable&&("auto"===scrollButtons&&scrollButtonsActive||!0===scrollButtons);conditionalElements.scrollButtonStart=scrollButtonsActive?(0,_jsxRuntime.jsx)(ScrollButtonComponent,(0,_extends2.default)({slots:{StartScrollButtonIcon:slots.StartScrollButtonIcon},
slotProps:{startScrollButtonIcon:startScrollButtonIconProps},orientation,direction:isRtl?"right":"left",onClick:handleStartScrollClick,disabled:!displayStartScroll},TabScrollButtonProps,{className:(0,_clsx.default)(classes.scrollButtons,TabScrollButtonProps.className)})):null;conditionalElements.scrollButtonEnd=scrollButtonsActive?(0,_jsxRuntime.jsx)(ScrollButtonComponent,(0,_extends2.default)({slots:{EndScrollButtonIcon:slots.EndScrollButtonIcon},slotProps:{endScrollButtonIcon:endScrollButtonIconProps},
orientation,direction:isRtl?"left":"right",onClick:handleEndScrollClick,disabled:!displayEndScroll},TabScrollButtonProps,{className:(0,_clsx.default)(classes.scrollButtons,TabScrollButtonProps.className)})):null;return conditionalElements})();return(0,_jsxRuntime.jsxs)(TabsRoot,(0,_extends2.default)({className:(0,_clsx.default)(classes.root,className),ownerState:props,ref,as:component},inProps,{children:[conditionalElements.scrollButtonStart,conditionalElements.scrollbarSizeListener,(0,_jsxRuntime.jsxs)(TabsScroller,
{className:classes.scroller,ownerState:props,style:{overflow:scrollerStyle.overflow,[vertical?`margin${isRtl?"Left":"Right"}`:"marginBottom"]:visibleScrollbar?void 0:-scrollerStyle.scrollbarWidth},ref:tabsRef,children:[(0,_jsxRuntime.jsx)(FlexContainer,{"aria-label":ariaLabel,"aria-labelledby":ariaLabelledBy,"aria-orientation":"vertical"===orientation?"vertical":null,className:classes.flexContainer,ownerState:props,onKeyDown:event=>{const list=tabListRef.current,currentFocus=(0,_ownerDocument.default)(list).activeElement;
...
indicatorColor:global.default.oneOfType([global.default.oneOf(["primary","secondary"]),global.default.string]),onChange:global.default.func,orientation:global.default.oneOf(["horizontal","vertical"]),ScrollButtonComponent:global.default.elementType,scrollButtons:global.default.oneOf(["auto",!1,!0]),selectionFollowsFocus:global.default.bool,slotProps:global.default.shape({endScrollButtonIcon:global.default.oneOfType([global.default.func,global.default.object]),startScrollButtonIcon:global.default.oneOfType([global.default.func,
global.default.object])}),slots:global.default.shape({EndScrollButtonIcon:global.default.elementType,StartScrollButtonIcon:global.default.elementType}),sx:global.default.oneOfType([global.default.arrayOf(global.default.oneOfType([global.default.func,global.default.object,global.default.bool])),global.default.func,global.default.object]),TabIndicatorProps:global.default.object,TabScrollButtonProps:global.default.object,textColor:global.default.oneOf(["inherit","primary","secondary"]),value:global.default.any,
variant:global.default.oneOf(["fullWidth","scrollable","standard"]),visibleScrollbar:global.default.bool};exports.default=require}
