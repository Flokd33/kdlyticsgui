shadow$provide.module$node_modules$vega_scenegraph$build$vega_scenegraph=function(global$jscomp$0,require,module,exports$jscomp$0){(function(global,factory){"object"===typeof exports$jscomp$0&&"undefined"!==typeof module?factory(exports$jscomp$0,require("module$node_modules$vega_util$build$vega_util"),require("module$node_modules$vega_canvas$build$vega_canvas"),require("module$node_modules$vega_loader$build$vega_loader"),require("module$node_modules$d3_shape$dist$d3_shape"),require("module$node_modules$d3_path$dist$d3_path")):
"function"===typeof define&&define.amd?define("exports vega-util vega-canvas vega-loader d3-shape d3-path".split(" "),factory):(global=global||self,factory(global.vega={},global.vega,global.vega,global.vega,global.d3,global.d3))})(this,function(exports,vegaUtil,vegaCanvas,vegaLoader,d3Shape,d3Path){function Bounds(b){this.clear();b&&this.union(b)}function gradientRef(g,defs,base){var id=g.id,type=g.gradient,prefix="radial"===type?"p_":"";id||(id=g.id="gradient_"+gradient_id++,"radial"===type?(g.x1=
get(g.x1,.5),g.y1=get(g.y1,.5),g.r1=get(g.r1,0),g.x2=get(g.x2,.5),g.y2=get(g.y2,.5),g.r2=get(g.r2,.5),prefix="p_"):(g.x1=get(g.x1,0),g.y1=get(g.y1,0),g.x2=get(g.x2,1),g.y2=get(g.y2,0)));defs[id]=g;return"url("+(base||"")+"#"+prefix+id+")"}function get(val,def){return null!=val?val:def}function Item(mark){this.mark=mark;this.bounds=this.bounds||new Bounds}function GroupItem(mark){Item.call(this,mark);this.items=this.items||[]}function ResourceLoader(customLoader){this._pending=0;this._loader=customLoader||
...
b$jscomp$0;item=$jscomp$destructuring$var6.x1;brush=$jscomp$destructuring$var6.x2;var y2=$jscomp$destructuring$var6.y2;for($jscomp$destructuring$var6=$jscomp$destructuring$var6.y1;$jscomp$destructuring$var6<=y2;++$jscomp$destructuring$var6)for(var x$298=item;x$298<=brush;++x$298)if(context$1.isPointInPath(x$298,$jscomp$destructuring$var6))return!0;return!1}}function intersectPoint(item,box){return box.contains(item.x||0,item.y||0)}function intersectRect(item,box){var x=item.x||0,y=item.y||0;return box.intersects(b$jscomp$0.set(x,
...
!1!==mark.interactive&&(!filter||filter(mark)))){var items=mark.items,type$299=mark.marktype;mark=items.length;var i=0;if("group"===type$299)for(;i<mark;++i)intersectGroup(items[i],box,filter,hits);else for(filter=Marks[type$299].isect;i<mark;++i)type$299=items[i],intersectItem(type$299,box,filter)&&hits.push(type$299)}return hits}function intersectGroup(group,box,filter,hits){filter&&filter(group.mark)&&intersectItem(group,box,Marks.group.isect)&&hits.push(group);var marks=group.items,n=marks&&marks.length;
if(n){var x$300=group.x||0;group=group.y||0;box.translate(-x$300,-group);for(var i=0;i<n;++i)intersectMark(marks[i],box,filter,hits);box.translate(x$300,group)}return hits}function intersectItem(item,box,test){var bounds=item.bounds;return box.encloses(bounds)||box.intersects(bounds)&&test(item,box)}function sceneEqual(a,b,key){return a===b?!0:"path"===key?pathEqual(a,b):a instanceof Date&&b instanceof Date?+a===+b:vegaUtil.isNumber(a)&&vegaUtil.isNumber(b)?1E-9>=Math.abs(a-b):a&&b&&(vegaUtil.isObject(a)||
