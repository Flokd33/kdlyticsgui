...
globalThis:global||self,factory((global.vega=global.vega||{},global.vega.transforms={}),global.vega,global.vega))})(this,function(exports,vegaDataflow,vegaUtil){function Bitmaps(){let width=8,data=[],seen=new Uint32Array(0),curr=array$jscomp$0(0,width),prev=array$jscomp$0(0,width);return{data:()=>data,seen:()=>{var JSCompiler_inline_result=seen;var length=data.length;JSCompiler_inline_result.length>=length||(length=new JSCompiler_inline_result.constructor(length),length.set(JSCompiler_inline_result),
JSCompiler_inline_result=length);return seen=JSCompiler_inline_result},add(array){for(let i=0,j=data.length,n=array.length,t;i<n;++i)t=array[i],t._index=j++,data.push(t)},remove(num,map){const n=data.length;num=Array(n-num);const reindex=data;let t,i,j;for(i=0;!map[i]&&i<n;++i)num[i]=data[i],reindex[i]=i;for(j=i;i<n;++i)t=data[i],map[i]?reindex[i]=-1:(reindex[i]=j,curr[j]=curr[i],prev[j]=prev[i],num[j]=t,t._index=j++),curr[i]=0;data=num;return reindex},size:()=>data.length,curr:()=>curr,prev:()=>
prev,reset:k=>prev[k]=curr[k],all:()=>257>width?255:65537>width?65535:4294967295,set(k,one){curr[k]|=one},clear(k,one){curr[k]&=~one},resize(n,m){if(n>curr.length||m>width)width=Math.max(m,width),curr=array$jscomp$0(n,width,curr),prev=array$jscomp$0(n,width)}}}function array$jscomp$0(n,m,array){n=(257>m?array8:65537>m?array16:array32)(n);array&&n.set(array);return n}function Dimension(index,i$jscomp$0,query){const bit=1<<i$jscomp$0;return{one:bit,zero:~bit,range:query.slice(),bisect:index.bisect,
index:index.index,size:index.size,onAdd(added,curr){var range=this.bisect(this.range,added.value);added=added.index;const lo=range[0];range=range[1];const n1=added.length;let i;for(i=0;i<lo;++i)curr[added[i]]|=bit;for(i=range;i<n1;++i)curr[added[i]]|=bit;return this}}}function ascending(a,b){return null==a||null==b?NaN:a<b?-1:a>b?1:a>=b?0:NaN}function descending(a,b){return null==a||null==b?NaN:b<a?-1:b>a?1:b>=a?0:NaN}function bisector(f){function left(a,x,lo=0,hi=a.length){if(lo<hi){if(0!==compare1(x,
x))return hi;do{const mid=lo+hi>>>1;0>compare2(a[mid],x)?lo=mid+1:hi=mid}while(lo<hi)}return lo}let compare1,compare2,delta;2!==f.length?(compare1=ascending,compare2=(d,x)=>ascending(f(d),x),delta=(d,x)=>f(d)-x):(compare1=f===ascending||f===descending?f:zero,delta=compare2=f);return{left,center:function(a,x,lo=0,hi=a.length){hi=left(a,x,lo,hi-1);return hi>lo&&delta(a[hi-1],x)>-delta(a[hi],x)?hi-1:hi},right:function(a,x,lo=0,hi=a.length){if(lo<hi){if(0!==compare1(x,x))return hi;do{const mid=lo+hi>>>
1;0>=compare2(a[mid],x)?lo=mid+1:hi=mid}while(lo<hi)}return lo}}}function zero(){return 0}function permute(source,keys){return Array.from(keys,key=>source[key])}function SortedIndex(){let index$jscomp$0=new Uint32Array(0),value$jscomp$0=[],size=0;return{insert:function(key,data,base){if(!data.length)return[];const n0=size,n1=data.length,addi=new Uint32Array(n1);let addv=Array(n1);var i$jscomp$0;for(i$jscomp$0=0;i$jscomp$0<n1;++i$jscomp$0)addv[i$jscomp$0]=key(data[i$jscomp$0]),addi[i$jscomp$0]=i$jscomp$0;
addv=sort(addv,addi);if(n0){key=value$jscomp$0;data=index$jscomp$0;value$jscomp$0=Array(n0+n1);index$jscomp$0=new Uint32Array(n0+n1);i$jscomp$0=addv;var value=value$jscomp$0,index=index$jscomp$0;let i0=0,i1=0,i;for(i=0;i0<n0&&i1<n1;++i)key[i0]<i$jscomp$0[i1]?(value[i]=key[i0],index[i]=data[i0++]):(value[i]=i$jscomp$0[i1],index[i]=addi[i1++]+base);for(;i0<n0;++i0,++i)value[i]=key[i0],index[i]=data[i0];for(;i1<n1;++i1,++i)value[i]=i$jscomp$0[i1],index[i]=addi[i1]+base}else{if(0<base)for(i$jscomp$0=
0;i$jscomp$0<n1;++i$jscomp$0)addi[i$jscomp$0]+=base;value$jscomp$0=addv;index$jscomp$0=addi}size=n0+n1;return{index:addi,value:addv}},remove:function(num,map){const n=size;let idx,i,j;for(i=0;!map[index$jscomp$0[i]]&&i<n;++i);for(j=i;i<n;++i)map[idx=index$jscomp$0[i]]||(index$jscomp$0[j]=idx,value$jscomp$0[j]=value$jscomp$0[i],++j);size=n-num},bisect:function(range,array){let n;array?n=array.length:(array=value$jscomp$0,n=size);return[bisectLeft(array,range[0],0,n),bisectRight(array,range[1],0,n)]},
reindex:function(map){for(let i=0,n=size;i<n;++i)index$jscomp$0[i]=map[index$jscomp$0[i]]},index:()=>index$jscomp$0,size:()=>size}}function sort(values,index){values.sort.call(index,(a,b)=>{a=values[a];b=values[b];return a<b?-1:a>b?1:0});return permute(values,index)}function CrossFilter(params){vegaDataflow.Transform.call(this,Bitmaps(),params);this._dims=this._indices=null}function ResolveFilter(params){vegaDataflow.Transform.call(this,null,params)}const array8=n=>new Uint8Array(n),array16=n=>new Uint16Array(n),
array32=n=>new Uint32Array(n),ascendingBisect=bisector(ascending),bisectRight=ascendingBisect.right,bisectLeft=ascendingBisect.left;bisector(function(x){return null===x?NaN:+x}).center;CrossFilter.Definition={type:"CrossFilter",metadata:{},params:[{name:"fields",type:"field",array:!0,required:!0},{name:"query",type:"array",array:!0,required:!0,content:{type:"number",array:!0,length:2}}]};vegaUtil.inherits(CrossFilter,vegaDataflow.Transform,{transform(_,pulse){return this._dims?_.modified("fields")||
...
null:data[k]})):(pulse.filter(pulse.ADD,pass),pulse.filter(pulse.REM,k=>(curr[k]&ignore)===mask?data[k]:null));return pulse.filter(pulse.SOURCE,t=>pass(t._index))}});exports.crossfilter=CrossFilter;exports.resolvefilter=ResolveFilter})}
